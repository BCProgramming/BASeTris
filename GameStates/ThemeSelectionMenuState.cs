...
namespace BASeTris.GameStates
{
    ...
    //
    public class PagedMenuOptionsState : MenuState
    {
        ...
        public int MaxPerPage { get; set; } = 12;
        ...
        protected void PartitionMenuItems(IStateOwner pOwner,List<MenuStateMenuItem> ItemOptions)
        {
            ...
            NextPageItem = new MenuStateTextMenuItem() { Text = "Next", TipText = "Next Page" };
             ...
             foreach (var designeritem in ItemOptions.Concat(new[] { PreviousPageItem,NextPageItem,CancelItem }))
            {
            ...
            Pages = Partition((from t in ItemOptions select (MenuStateMenuItem)t).ToList(), MaxPerPage).ToList();
            PreparePage(CurrentPage);
            base.MenuItemActivated -= PagedMenuOptionsState_MenuItemActivated;
            base.MenuItemActivated += PagedMenuOptionsState_MenuItemActivated;
        }
        public PagedMenuOptionsState(IStateOwner pOwner,IBackground pBG,GameState pRevertState,  List<MenuStateMenuItem> ItemOptions,String pTitle)
        {
            _Owner= pOwner;
            ...
            if (ItemOptions != null)
            {
                PartitionMenuItems(pOwner, ItemOptions);
            }
        ...
        private void PagedMenuOptionsState_MenuItemActivated(object? sender, MenuStateMenuItemActivatedEventArgs e)
        {
