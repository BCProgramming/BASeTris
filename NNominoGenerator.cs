...
namespace BASeTris
{
    ...
    public class NNominoGenerator
    {
        ...
        private List<NominoPoint> ResetTranslation(List<NominoPoint> input)
        {
        ...
        {
            if (CurrentBuild!=null && CurrentBuild.Count > 5)
            {
                ;
            }
            if (CurrentBuild == null)
            {
                ...
                CurrentBuild = new List<NominoPoint>() {new NominoPoint(0, 0), new NominoPoint(0, 1) }; //create first two blocks
